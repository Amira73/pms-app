...
export default function AuthLayout() {
  ...
  const { pathname } = useLocation();
   ...
   const topImg =
    pathname.includes("/auth/register") ? IMAGES.auth2 :
  
    pathname.includes("/auth/login") ? IMAGES.auth5 :
  ...
  const bottomImg =
    pathname.includes("/auth/register") ? IMAGES.auth1 :
   
    pathname.includes("/auth/login") ? IMAGES.auth1 :
     pathname.includes("/auth/forget-password") ? IMAGES.auth6 :
       pathname.includes("/auth/reset-password") ? IMAGES.auth9 :
        pathname.includes("/auth/change-password") ? IMAGES.auth10 :
    IMAGES.auth1;
  return (
